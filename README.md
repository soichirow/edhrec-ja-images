...
- 横長サムネイルなどカード形ではない表示は、潰れ防止のため差し替え対象外
...
操作バーはカード画像の下に表示する設計です。EDHREC本来の価格やsynergy表示がある場合は、その表示の下へ操作バーを追加します。横長に表示されるサムネイルは、カード画像が潰れて読みにくくなるため差し替えません。EDHREC側のHTML構造が大きく変わった場合は、Issueで対象ページURLを共有してください。
