...
- スクロール前にカード情報と日本語画像を先読みする
- 日本語画像が見つかるまでは、EDHRECの元の英語画像をそのまま表示する
- 日本語印刷版が見つからないカードはScryfallの英語通常版を表示する
- 差し替えたカード名と操作ボタンを画像の下に表示する
- 日本語名をクリックするとScryfallの該当ページを開ける
- `コピー` ボタンで日本語名をコピーできる
- `★` ボタンでお気に入り登録できる
- 晴れる屋、BIG MAGIC、シングルスター、東京MTG、メルカリの検索リンクをカードごとに開ける
...
EDHREC側のHTML構造が大きく変わると、一部のカードでうまく差し替わらない可能性があります。また、Scryfallに日本語印刷版の画像がないカードは、Scryfallの英語通常版へ差し替えます。
...
一時的な通信エラーやScryfall側の5xx系エラーが出た場合は、少し時間をおいて最大2回まで再試行します。`429 Too Many Requests` が返ってきた場合は、`Retry-After` があればその待機時間を尊重します。
...
メルカリへのリンクは通常の検索リンクにしています。メルカリアンバサダーのアフィリエイトリンクを使う場合は、公式の規約とガイドラインを確認し、自分が管理するブログやSNSなどの媒体で、必要なPR表記を添えて利用してください。
...
最初はカード画像の上に日本語名やボタンを重ねる形も考えました。

でもカード画像の上に重ねると、カード名やボタンが読みにくくなることがあります。

そこで、カード画像の下に小さな操作バーを差し込む形にしました。
...
表示確認用のテストページでは、操作バーが画像や英語名に重ならないことを確認しています。
